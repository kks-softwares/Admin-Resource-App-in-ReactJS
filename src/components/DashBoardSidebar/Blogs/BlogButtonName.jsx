...
export default function BlogButtonName({ data, index, setAllCbutton }) {
  ...
  const handledeleteBlog = () => {
    axios
      .post(`${API_HOST}/callToActionButton/removeButton`, {
  ...
  const handleeditBlog = () => {
    axios
      .post(`${API_HOST}/callToActionButton/editButton`, {
        buttonId: data?.buttonId,
        callToActionButton: titleuser,
      })
      .then(() => {
        axios.get(`${API_HOST}/callToActionButton/viewButton`).then((res) => {
            setAllCbutton(res?.data?.success?.data);
  ...
  return (
    <div>
      <div style={{ alignItems: "center" }} className="navoftableblogsdata">
        ...
        <div style={{ width: "70vw" }}>{data?.callToActionButton}</div>
